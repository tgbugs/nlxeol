...
for prefix, outer_identifiers in data.items():
    ...
    for id_ in outer_identifiers:
        ...
        #a future tool
        for index, value in enumerate(js['LABELS']):
            #if value == 'PREFIX':
            #print(js[id_][0][index])
            if "http://ontology.neuinfo.org/NIF/Backend/BIRNLex_annotation_properties.owl#Bill_Bug" in js[id_][0][index]:
                person = ns("http://ontology.neuinfo.org/NIF/Backend/BIRNLex_annotation_properties.owl#Bill_Bug")
                js[id_][0][index] = person
                print(js[id_][0][index])

#useful to make temp RDFs
#for i in js['LABELS']:
